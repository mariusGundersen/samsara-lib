...
export default async function () {
  //init
  await fs.mkdir(spiritsDir(), { recursive: true });
...
async function upgradeFromJsonToYaml() {
  ...
  await Promise.all(
    spirits.map(async function (spirit) {
      ...
      if (jsonExists) {
        const json = await fs.readFile(
          spiritContainerConfigJson(spirit),
          "utf8"
        );
      ...
      const lives = await filterAsync(files, (life) =>
        isDirectory(spiritLife(spirit, life))
      );
      await Promise.all(
        lives.map(async function (life) {
          const yamlExists = await isFile(
            spiritLifeContainerConfig(spirit, life)
          );
          const jsonExists = await isFile(
            spiritLifeContainerConfigJson(spirit, life)
          );
...
function filterAsync(list, test) {
  return Promise.all(
    list.map((value) =>
      test(value).then((include) => ({ value: value, include: include }))
    )
  ).then((list) =>
    list
...
function createYamlConfig(name, config) {
  ...
  yamlConfig.environment = Object.keys(config.env || {}).map((key) => ({
    key: key,
    value: config.env[key],
  }));
  yamlConfig.volumes = Object.keys(config.volumes || {}).map(
    (containerPath) => ({
  ...
  yamlConfig.ports = Object.keys(config.ports || {}).map((hostPort) => ({
    hostPort: hostPort,
    containerPort: config.ports[hostPort].containerPort,
    hostIp: config.ports[hostPort].hostIp,
  }));
  yamlConfig.links = Object.keys(config.links || {}).map((alias) => ({
    alias: alias,
    spirit: config.links[alias].spirit || "",
    container: config.links[alias].container || "",
  }));
  yamlConfig.volumesFrom = (config.volumesFrom || []).map((entry) => ({
    spirit: entry.spirit || "",
