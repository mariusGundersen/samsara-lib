...
import { filter } from "./util/asyncArray.js";
import statusToState, { DEAD, RUNNING } from "./util/statusToState.js";
  ...
  return await Promise.all(
    directories
      ...
      .map(async function (name) {
        ...
        const isDeploying = await fs.stat(spiritDeployLock(name)).then(
          (stat) => stat.isFile(),
          (e) => false
        );
        if (isDeploying) {
...
async function getLives(name, containers) {
  ...
  const lives = containers
    ...
    .map((container) => ({
      ...
      uptime:
        (/^(Exited\s\(\d+\)\s|^Up\s)(.*)/.exec(container.Status) || [])[2] ||
        "",
    }));
  ...
  return lives
    .concat(
      directories
        .filter(
          (directory) =>
            lives.map((life) => life.life * 1).indexOf(directory * 1) === -1
        )
        .map((directory) => ({
          life: directory * 1,
          state: DEAD,
...
function getCurrentLife(lives) {
  return (
    lives.filter((life) => life.state === RUNNING).reverse()[0] ||
    lives.map((x) => x).reverse()[0] || { life: "?", state: DEAD, uptime: "" }
  );
}
